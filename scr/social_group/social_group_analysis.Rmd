...
# Richness Diversity 
# Compute richness (count of taxa present in each sample)
richness <- rowSums(euks.social.clean > 0)

# Create richness table
richness_table <- tibble(
  Sample_ID = names(richness),
  Richness = as.numeric(richness)
)

# Merge with metadata
richness_table.social.meta <- left_join(richness_table, 
                         euks.social.metadata %>% select(Sample_ID, Social_group),
                                        by = "Sample_ID")

# Define color palette
colors <- c("darkorange", "#6A27E1")

# Boxplot
richness_plot <- ggplot(richness_table.social.meta, 
                        aes(x = Social_group, y = Richness, fill = Social_group)) +
  geom_boxplot(alpha = 0.7, outlier.size = -1) +
  labs(title = "Taxonomic Richness Comparison", y = "Taxonomic Richness") +
  theme_minimal() +
  scale_fill_manual(values = colors) +
  annotate("text", x = 1.5, y = max(richness_table.social.meta$Richness), 
           label = paste("p =", round(richness_p_value, 3)), size = 5, hjust = 0.5)
print(richness_plot)

# save plot
pdf(file=paste0("figures/social_group/taxonomic_richness_plot.pdf"), width = 4, height=4)
richness.plot
dev.off()
...
# add social group information
nmds_df.meta <- left_join(nmds_df, euks.social.metadata %>% select(Sample_ID, Social_group, Sex, Age, Number.of.paired.end.sequences))
...
# Perform PERMANOVA
permanova_result <- adonis(bray_curtis_dist ~ socialgroup + Age + sex + Number.of.paierd-end.sequences, data = samples_with_species)

# results
permanova_table <- as.data.frame(permanova_result$aov.tab)

```
#Grooming Network Analysis
#import data
viola_groom<-read.table("Viola_1yrgroom_forMantel.csv", header=F, sep=',')
viola_jaccard<-read.table("Viola_jaccard_distance_matrix_forMantel.csv", header=F, sep=',')

mica_groom<-read.table("Mica_1yrgroom_forMantel.csv", header=F, sep=',')
mica_jaccard<-read.table("Mica_jaccard_distance_matrix_forMantel.csv", header=F, sep=',')

# Convert to matrix
viola_groom <- as.matrix(viola_groom)
viola_jaccard <- as.matrix(viola_jaccard)

mica_groom <- as.matrix(mica_groom)
mica_jaccard <- as.matrix(mica_jaccard)
...
# Run the Mantel test
viola_mantel <- mantel(viola_groom, viola_jaccard)
mica_mantel <- mantel(mica_groom, mica_jaccard)
...
# plot the results
plot(viola_groom, viola_jaccard)
plot(mica_groom, mica_jaccard)
